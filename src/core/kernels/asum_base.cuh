...
#undef GEN_SPGPU_ASUM_NAME
#define GEN_SPGPU_ASUM_NAME(x) CONCAT(CONCAT(spgpu,x),asum)
...
__device__ __host__ static float float_add(float a, float b) { 
#ifndef __CUDA__ARCH__
	return a + b;
#else
	return PREC_FADD (a, b); 
#endif	
}
...
__device__ __host__ static double double_add(double a, double b) { 
#ifndef __CUDA__ARCH__
	return a + b;
#else
	return PREC_DADD (a, b); 
#endif
}
...
static __device__ CONCAT(absType_,VALUE_TYPE) CONCAT(TYPE_SYMBOL,asumReductionResult)[128];


static __device__ CONCAT(absType_,VALUE_TYPE) asumvv(VALUE_TYPE a, VALUE_TYPE b)
{
...
static __device__ __host__ CONCAT(absType_,VALUE_TYPE) asumaa(CONCAT(absType_,VALUE_TYPE) a, CONCAT(absType_,VALUE_TYPE) b)
{
	return a + b;
}

static __device__ CONCAT(absType_,VALUE_TYPE) asumav(CONCAT(absType_,VALUE_TYPE) a, VALUE_TYPE b)
{
	CONCAT(absType_,VALUE_TYPE) absb = CONCAT(abs_,VALUE_TYPE)(b);
...
__global__ void 
CONCAT(GEN_SPGPU_ASUM_NAME(TYPE_SYMBOL),_kern)
(int n, VALUE_TYPE* x)
{
	...
	if (threadIdx.x < 32) 
	{
		...
		if (threadIdx.x < 16) asumaa(vsSum[threadIdx.x], vsSum[threadIdx.x + 16]);
		if (threadIdx.x < 8) asumaa(vsSum[threadIdx.x], vsSum[threadIdx.x + 8]);
		if (threadIdx.x < 4) asumaa(vsSum[threadIdx.x], vsSum[threadIdx.x + 4]);
		if (threadIdx.x < 2) asumaa(vsSum[threadIdx.x], vsSum[threadIdx.x + 2]);
		if (threadIdx.x == 0)
			CONCAT(TYPE_SYMBOL,asumReductionResult)[blockIdx.x] = asumaa(vsSum[0], vsSum[1]);
...
GEN_SPGPU_ASUM_NAME(TYPE_SYMBOL)
(spgpuHandle_t handle, int n, VALUE_TYPE* x)
{
#ifdef USE_CUBLAS
	CONCAT(absType_,VALUE_TYPE) res = CONCAT(cublas,CONCAT(TYPE_SYMBOL,asum))(n,x,1);
	...
	CONCAT(GEN_SPGPU_ASUM_NAME(TYPE_SYMBOL),_kern)<<<blocks, BLOCK_SIZE, 0, handle->currentStream>>>(n, x);;
	cudaMemcpyFromSymbol(tRes, CONCAT(TYPE_SYMBOL,asumReductionResult), blocks*sizeof(CONCAT(absType_,VALUE_TYPE)));

	for (int i=0; i<blocks; ++i)
	{
		res = asumaa(res, tRes[i]);
	}

	cudaCheckError("CUDA error on asum");
