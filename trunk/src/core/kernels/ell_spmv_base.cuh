...
__device__ __host__ float zero_float() { return 0.0f; }
__device__ __host__ cuFloatComplex zero_cuFloatComplex() { return make_cuFloatComplex(0.0, 0.0); }
__device__ __host__ bool float_isNotZero(float x) { return x != 0.0f; }

__device__ float float_fma(float a, float b, float c) { return PREC_FADD(PREC_FMUL (a, b), c); }
__device__ float float_add(float a, float b) { return PREC_FADD (a, b); }
__device__ float float_mul(float a, float b) { return PREC_FMUL (a, b); }

__device__ cuFloatComplex cuFloatComplex_fma(cuFloatComplex a, cuFloatComplex b, cuFloatComplex c) { return cuCfmaf(a, b, c); } 
__device__ cuFloatComplex cuFloatComplex_add(cuFloatComplex a, cuFloatComplex b) { return cuCaddf(a, b); }
__device__ cuFloatComplex cuFloatComplex_mul(cuFloatComplex a, cuFloatComplex b) { return cuCmulf(a, b); }

__device__ float readValue_float(float fetch) { return fetch; }
__device__ cuFloatComplex readValue_cuFloatComplex(cuFloatComplex fetch) { return fetch; }

// host or c.c >= 1.3 
#if (__CUDA_ARCH__ >= 130) || (!__CUDA_ARCH__)
__device__ __host__ double zero_double() { return 0.0; }
__device__ __host__ cuDoubleComplex zero_cuDoubleComplex() { return make_cuDoubleComplex(0.0, 0.0); }
__device__ __host__ bool double_isNotZero(double x) { return x != 0.0; }

__device__ double double_fma(double a, double b, double c) { return PREC_DADD(PREC_DMUL (a, b), c); }
__device__ double double_add(double a, double b) { return PREC_DADD (a, b); }
__device__ double double_mul(double a, double b) { return PREC_DMUL (a, b); }

__device__ cuDoubleComplex cuDoubleComplex_fma(cuDoubleComplex a, cuDoubleComplex b, cuDoubleComplex c) { return cuCfma(a, b, c); }
__device__ cuDoubleComplex cuDoubleComplex_add(cuDoubleComplex a, cuDoubleComplex b) { return cuCadd(a, b); }
__device__ cuDoubleComplex cuDoubleComplex_mul(cuDoubleComplex a, cuDoubleComplex b) { return cuCmul(a, b); }

__device__ double readValue_double(int2 fetch) { return __hiloint2double (fetch.y, fetch.x); }
__device__ cuDoubleComplex readValue_cuDoubleComplex(int4 fetch) 
{
	cuDoubleComplex c;
...
__device__ VALUE_TYPE fetchTex(int pointer)
{
